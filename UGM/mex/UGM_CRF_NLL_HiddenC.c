...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    double obs, *g, *nodeBel, *edgeBel, *nodeBelC, *edgeBelC, *Xnode, *Xedge;
    ...
    nodeBelC = mxGetPr(prhs[11]);
    edgeBelC = mxGetPr(prhs[12]);
            ...
            for (s2 = 0; s2 < nStates[n2]; s2++) {
                for (f = 0; f < nEdgeFeatures; f++) {
                    if (edgeMap[s1 + maxState*(s2 + maxState*(e + nEdges*f))] > 0) {
                        ...
                        else if (Y[i + nInstances*n1] == 0 && Y[i + nInstances*n2]-1 == s2) {
                            obs = edgeBelC[s1 + maxState*(Y[i + nInstances*n2] + maxState*e)];
                        }
                        else if (Y[i + nInstances*n1]-1 == s1 && Y[i + nInstances*n2] == 0) {
                            obs = edgeBelC[Y[i + nInstances*n1] + maxState*(Y[i + nInstances*n2] + maxState*e)];
                        }
