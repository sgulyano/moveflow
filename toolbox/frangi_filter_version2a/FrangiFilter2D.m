function [outIm,whatScale,Direction,outBr] = FrangiFilter2D(I, options)
...
ALLbranch=zeros([size(I) length(sigmas)]);
...
for i = 1:length(sigmas),
    ...
    if(options.BlackWhite)
        B2 = sqrt(max(Lambda2,0) .* max(Lambda1,0));
    else
        B2 = sqrt(min(Lambda2,0) .* min(Lambda1,0));
    end
   
    % Compute the output image
    Ifiltered = exp(-Rb/beta) .*(1-exp(-S2/c));
    Ibranch = (1-exp(-B2/beta)) .*(1-exp(-S2/c));
...
if length(sigmas) > 1,
    ...
    if(nargout>3)
        outBr = reshape(ALLbranch,size(I));
    end
end
