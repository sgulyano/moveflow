...
void mexFunction( int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[] ) {
    ...
    double *Dvecx, *Dvecy, *Dvecz,*Dvecx1, *Dvecy1, *Dvecz1,*Dvecx2, *Dvecy2, *Dvecz2, *Deiga, *Deigb, *Deigc;

    float *Dxx_f, *Dxy_f, *Dxz_f, *Dyy_f, *Dyz_f, *Dzz_f;
    float *Dvecx_f, *Dvecy_f, *Dvecz_f,*Dvecx1_f, *Dvecy1_f, *Dvecz1_f,*Dvecx2_f, *Dvecy2_f, *Dvecz2_f, *Deiga_f, *Deigb_f, *Deigc_f;
    ...
    if(nrhs!=6) {
        ...
        mexErrMsgTxt("Three or Six or Twelve outputs are required");
    }
    ...
    if(mxGetClassID(prhs[0])==mxDOUBLE_CLASS) {
       /* Assign pointers to each input. */
        ...
        if(nlhs>=6) {
            /* Main direction (largest eigenvector) */
            ...
            Dvecx = mxGetPr(plhs[3]);
			Dvecy = mxGetPr(plhs[4]); 
			Dvecz = mxGetPr(plhs[5]);
			if(nlhs==12){
				...
				Dvecx1 = mxGetPr(plhs[6]);
				Dvecy1 = mxGetPr(plhs[7]); 
				Dvecz1 = mxGetPr(plhs[8]);
				Dvecx2 = mxGetPr(plhs[9]);
				Dvecy2 = mxGetPr(plhs[10]); 
				Dvecz2 = mxGetPr(plhs[11]);
			}
        }
       ...
       /* Assign pointers to each input. */
        ...
        if(nlhs>=6) {
			...
			if(nlhs==12){
				...
				Dvecx1_f = (float *)mxGetPr(plhs[6]);
				Dvecy1_f = (float *)mxGetPr(plhs[7]); 
				Dvecz1_f = (float *)mxGetPr(plhs[8]);
				Dvecx2_f = (float *)mxGetPr(plhs[9]);
				Dvecy2_f = (float *)mxGetPr(plhs[10]); 
				Dvecz2_f = (float *)mxGetPr(plhs[11]);
			}
        }
        ...
        for(i=0; i<npixels; i++) {
            ...
            if(nlhs>=6) {
				...
				if(nlhs==12) {
					Dvecx1_f[i]=(float)Davec[0][1];
			        Dvecy1_f[i]=(float)Davec[1][1];
			        Dvecz1_f[i]=(float)Davec[2][1];
					Dvecx2_f[i]=(float)Davec[0][2];
			        Dvecy2_f[i]=(float)Davec[1][2];
			        Dvecz2_f[i]=(float)Davec[2][2];
				}
            }
        }
