...
for fr = st_fr:23
    ...
    % draw soma
    I3D((xx3 - ddadX - padsize(2)).^2 + (yy3 - ddadY - padsize(1)).^2 + 4*(zz3 - ddadZ - padsize(3)).^2 < (swc_ddad(1,6) + 1)^2) = 1;
    % get 3D isosurface
    ...
    I3D = imdilate(I3D, ones(3,3,3));
    % draw soma
    I3D((xx3 - ddaeX - padsize(2)).^2 + (yy3 - ddaeY - padsize(1)).^2 + 4*(zz3 - ddaeZ - padsize(3)).^2 < (swc_ddae(1,6) + 1)^2) = 1;    
...
%         % move img
%         [yy,~,~] = ind2sub(size(masks), find(masks));
%         zoff = padsize(3)+1 - ceil(num_slice/2);
%         if zoff > 0
%             imgmove = imtranslate(img,[0,0,zoff],'OutputView','full');
%             if size(masks,3) > size(imgmove,3)
%                 imgcrop = img(min(yy):max(yy),newsomaX-xoffset1:newsomaX+xoffset2,:);
%                 imgmove = padarray(imgcrop, [0 0 ceil((size(masks,3)-size(imgcrop,3))/2)], 'both');
%                 imgmove = imgmove(:,:,1:size(masks,3));
%                 maskcrop = imgaussfilt3(double(masks(min(yy):max(yy),newsomaX-xoffset1:newsomaX+xoffset2,:)), 1);
%             else
%                 error('wrong size');
%             end
%         else
...
%         figure(20);
%         subplot(2,2,3); imagesc(max(maskcrop,[],3));
%         subplot(2,2,4); imagesc(max(imgmove,[],3));
%         
%         aa = maskcrop ./ max(maskcrop(:));
%         bb = double(imgmove) ./ max(double(imgmove(:)));
%         mi_score(fr) = mean(aa(:).*bb(:));
%         mi_score(fr) = max(mi(maskcrop,imgmove,64),0);
