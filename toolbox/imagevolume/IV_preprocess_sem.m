function [Vrm, Iout, scale] = IV_preprocess_sem( V, ZRATIO, varargin )
...
figure(1); imshow3D(V);
%%
disp('Preprocess for SEM Dataset');
V_adj = zeros(size(V));
for fcount = 1:FNUM
    V_adj(:,:,fcount) = imadjust(uint8(V(:,:,fcount)),[0.07,0.2],[0,1]);
end
% figure(2); imshow3D(V_adj);
% figure, imshow(max(V_adj,[],3),[])
