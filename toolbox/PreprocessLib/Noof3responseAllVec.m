...
%  planar) and the smallest detection radius is not smaller than sigma (i.e. 
...
function [output,LambdaOut1,LambdaOut2,LambdaOut3,Voutx,Vouty,Voutz]= Noof3responseAllVec(image, radii, options)
tic
...
if exist('options', 'var')~=0
        ...
        if isfield(options, 'marginwidth')~=0
...
%     marginwidth = [ceil((max(radii)+sigma*3)/pixelspacing(1)) ceil((max(radii)+sigma*3)/pixelspacing(2)) ceil((max(radii)+sigma*3)/pixelspacing(3)) ];
    ...
    % equivalent buffered variables inside the following for-loop. It 
...
% save memory, although the code looks clumsy.
...
% End of in-place FFT
        ...
        % individually. CPU works better if data is fed as a whole block.
...
%    [eigenvalue1,eigenvalue2,eigenvalue3] = eigenvaluefield33(outputfeature_11, outputfeature_12, outputfeature_13, outputfeature_22, outputfeature_23, outputfeature_33);
        [mine,mide,maxe,Vx,Vy,Vz]=eig3volume(outputfeature_11, outputfeature_12, outputfeature_13, outputfeature_22, outputfeature_23, outputfeature_33);
%         clear mine;
...
% Feel free to change the combination of the eigenvalues, just as vesselness mesaure        
        switch rtype
            case 0,
                tmpfeature = maxe;
            case 1,
            ...
            case 6
                ...
                Voxel_data(mide > 0)=0; Voxel_data(maxe > 0)=0;
                            tmpfeature = Voxel_data;
            case 7
                tmpfeature = sqrt(mine.^2+mide.^2+maxe.^2);
            case 8
                tmpfeature = sqrt(abs(maxe.*mide));
                tmpfeature(maxe<0)=0;
                tmpfeature(mide<0)=0;
...
%         clear mide;
% Select the voxelwise responses according to the largest magnitude response
     ...
     if i==1
        ...
        LambdaOut1(condition)=mine(condition);
        LambdaOut2(condition)=mide(condition);
        LambdaOut3(condition)=maxe(condition); 
        Voutx(condition)=Vx(condition);
        Vouty(condition)=Vy(condition);
        Voutz(condition)=Vz(condition);
