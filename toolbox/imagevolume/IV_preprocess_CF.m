function [Vrm, Iout, scale] = IV_preprocess_CF( V, ZRATIO, varargin )
%IV_PREPROCESS_CF preprocess step for image volume of CF from DIADEM
...
V_adj = zeros(size(V));
for fcount = 1:FNUM
    V_adj(:,:,fcount) = imadjust(uint8(V(:,:,fcount)),[0,0.5],[0,1]);
end
