function H = AC_quiver(Fextx, Fexty, I, color)
...
%% inputs check
H = NaN;
if ~ismember(nargin, 2:4) || ~ismatrix(Fextx) || ~ismatrix(Fextx)
    error('Invalid inputs to AC_QUIVER!')
end

if nargin == 2
    if any(size(Fextx) ~= size(I)) || any(size(Fexty) ~= size(I)) 
