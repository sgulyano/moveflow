function [Iout,whatScale,Voutx,Vouty,Voutz]=NFrangiGVF3D(ux,uy,uz,vy,vz,wz,options,m,n,o)
%
...
%     [Dxx, Dyy, Dzz, Dxy, Dxz, Dyz] = NGVFHessian3D(I,sigmas(i));
...
%     end
    ...
    if(nargout>2)
        [Lambda1,Lambda2,Lambda3,Vx,Vy,Vz]=eig3volume(ux,uy,uz,vy,vz,wz);
    else
        [Lambda1,Lambda2,Lambda3]=eig3volume(ux,uy,uz,vy,vz,wz);
    end
    ...
    if(i==1)
        Iout=Voxel_data;
        if(nargout>1)
            whatScale = ones([m,n,o],class(Iout));
        end
        if(nargout>2)
